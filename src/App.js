...
const CONTRACT_ADDRESS = "0x42FF8F4db315408B4bF0C42c98a4132d8b8dD6f6";
  ...
  const mintTokens = async () => {
    ...
    try {
      ...
      if (ethereum) {
        ...
        let tx = await contract.mint(currentAccount, tokenCount);
    ...
    if (totalSupply) {
      ...
      return (
        <div className="mint-container">
          ...
          {!!ownerAddress && (
            <>
              <p className="owner">Owner: {ownerAddress}</p>
              <br />
              <p className="note">(Only the owner can mint tokens!)</p>
            </>
          )}
        </div>
      );
